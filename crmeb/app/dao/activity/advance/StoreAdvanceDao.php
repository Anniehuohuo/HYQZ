    ...
    public function getList(array $where, int $page = 0, int $limit = 0)
    {
        return $this->search($where, false)
            ->when($where['time_type'], function ($query) use ($where) {
                $now = time();
                if ($where['time_type'] == 1) $query->where('start_time', '>', $now);
                if ($where['time_type'] == 2) $query->where('start_time', '<=', $now)->where('stop_time', '>=', $now);
                if ($where['time_type'] == 3) $query->where('stop_time', '<', $now);
            })
