<script>
	...
	export default {
		...
		onShow() {
			...
			try {
				queryData = uni.getEnterOptionsSync() || queryData
			} catch (e) {
				try {
					// #ifdef MP
					queryData = wx.getEnterOptionsSync() || queryData
					// #endif
				} catch (e2) {}
			}
		...
		async onLaunch(option) {
			...
			basicConfig().then((res) => {
				if (res && res.data) uni.setStorageSync('BASIC_CONFIG', res.data);
			}).catch(() => {});
			// #ifdef H5
			...
			colorChange('color_change').then((res) => {
				...
				const themeKey = themeMap[status] || 'hyqz';
			...
			try {
				startParamObj = wx.getEnterOptionsSync() || startParamObj
			} catch (e) {}
			...
			try {
				...
				version = (sys && sys.SDKVersion) ? sys.SDKVersion : version
			} catch (e) {}
			...
			getCrmebCopyRight().then((res) => {
				if (res && res.data) uni.setStorageSync('copyRight', res.data);
			}).catch(() => {});
		},
		// #ifdef H5
		onHide() {
